...
const Cart = () => {
  ...
  const handleQuantityChange = (itemId, newQuantity) => {
    const parsedQuantity = parseInt(newQuantity, 10); // Convert input to an integer
    ...
    const newCartItems = cartItems.map((item) => {
      if (item.id === itemId) {
        ...
        if (parsedQuantity > item.stock) {
          ...
          return item; // Do not update if quantity exceeds stock
        }
